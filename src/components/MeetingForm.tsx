	...
	return (
		<div className="border-app-border rounded-[10px] border bg-white p-5 px-6">
			<div className="mb-4 flex items-center justify-between">
				...
				<div className="flex items-center gap-2">
					...
					<button
						type="button"
						onClick={resetForm}
						className="text-app-dark border-app-border cursor-pointer rounded-md border bg-white px-3.5 py-1.5 text-sm transition-colors hover:bg-[#F0F8FB]"
					>
						Clear
					</button>
					<button
						type="submit"
						form="meeting-form"
						className="bg-app-blue hover:bg-app-navy cursor-pointer rounded-md px-3.5 py-1.5 text-sm font-semibold text-white transition-colors"
					>
						Save Meeting
					</button>
				</div>
			...
			<form id="meeting-form" onSubmit={handleSubmit} noValidate>
				<div className="grid grid-cols-2 items-start gap-4 lg:grid-cols-4 xl:grid-cols-6">
					{/* Title */}
					<div className="col-span-2">
						<label className="text-app-dark mb-1 block text-xs font-semibold">
						...
						<div className="mt-1 flex flex-wrap gap-1.5">
							{DAYS.map((day) => {
								...
								return (
									<label
										key={day}
										className={`inline-flex cursor-pointer items-center gap-1 rounded-md border px-2.5 py-1.5 text-xs font-semibold transition-all select-none ${
											checked
							...
							<button
								...
								className="border-app-border text-app-blue hover:border-app-muted cursor-pointer rounded-md border bg-white px-2.5 py-1.5 text-xs font-semibold transition-colors hover:bg-[#F0F8FB]"
							>
								M–F
							</button>
						</div>
