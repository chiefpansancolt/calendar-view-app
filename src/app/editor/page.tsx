...
export default function EditorPage() {
	...
	function showPrompt(
		onConfirm: (value: string) => void,
		opts?: { title?: string; message?: string; defaultValue?: string }
	) {
	...
	function handleDeleteCategory(name: string) {
		...
		if (count > 0) {
			showConfirm(`"${name}" is used by ${count} meeting(s). Remove anyway?`, doDelete, {
						...
						if (existing.meetings.length === 0) {
							...
							const doMerge = () => {
								...
								const existingPeopleNames = new Set(
									existing.people.map((p) => p.name)
								);
								saveClientData(targetClient, {
									meetings: [
										...existing.meetings,
										...pd.parsedData.meetings.filter(
											(m) => !existingIds.has(m.id)
										),
									],
									categories: [
										...existing.categories,
										...(pd.parsedData.categories || []).filter(
											(c) => !existingCatNames.has(c.name)
										),
									],
									people: [
										...existing.people,
										...(pd.parsedData.people || []).filter(
											(p) => !existingPeopleNames.has(p.name)
										),
									],
								});
								setClientListState(list);
		...
		<div className="bg-app-gray min-h-screen">
			...
			>
				<button
					...
					className="text-app-dark border-app-border flex cursor-pointer items-center gap-2 rounded-md border bg-white px-3.5 py-1.5 text-sm transition-colors hover:bg-[#F0F8FB]"
				>
				...
				<Link
					href="/"
					className="bg-app-blue hover:bg-app-navy flex cursor-pointer items-center gap-2 rounded-md px-3.5 py-1.5 text-sm font-semibold text-white transition-colors"
				>
